# R055 — 522434c1dd4f [MEDIUM][partial]

**Subject**: server: op->error() dispatch  
**Date**: 2023-07-03  
**pvxs SHA**: 522434c1dd4f  

## pva-rs mapping
- crates/epics-pva-rs/src/server_native/source.rs
- crates/epics-pva-rs/src/server_native/tcp.rs (GET handler)
- crates/epics-pva-rs/src/server_native/tcp.rs (MONITOR handler)

## Verdict
**partial** — MEDIUM

> TODO: manual review — does this bug exist in pva-rs?

## Diff
```diff
commit 522434c1dd4f8016830ee0e5cc3311e68bef33e3
Author: Michael Davidsaver <mdavidsaver@gmail.com>
Date:   Mon Jul 3 08:08:59 2023 -0700

    server: op->error() dispatch
    
    Avoid blocking in error()
---
 src/pvxs/source.h    | 2 ++
 src/pvxs/srvcommon.h | 1 +
 src/serverget.cpp    | 9 ++++++---
 src/servermon.cpp    | 5 +++--
 4 files changed, 12 insertions(+), 5 deletions(-)

diff --git a/src/pvxs/source.h b/src/pvxs/source.h
index cdc868d..7b7b109 100644
--- a/src/pvxs/source.h
+++ b/src/pvxs/source.h
@@ -29,8 +29,9 @@ public:
     //! For GET_FIELD, GET, or PUT.  Inform peer of our data-type.
     //! @throws std::runtime_error if the client pvRequest() field mask does not select any fields of prototype.
     virtual void connect(const Value& prototype) =0;
     //! Indicate that this operation can not be setup
+    //! @since UNRELEASED Does not block
     virtual void error(const std::string& msg) =0;
 
     ConnectOp(const std::string& name,
            const std::shared_ptr<const ClientCredentials>& cred, op_t op,
@@ -144,8 +145,9 @@ public:
     //! @throws std::runtime_error if the client pvRequest() field mask does not select any fields of prototype.
     virtual std::unique_ptr<MonitorControlOp> connect(const Value& prototype) =0;
 
     //! Indicate that this operation can not be setup
+    //! @since UNRELEASED Does not block
     virtual void error(const std::string& msg) =0;
 
     MonitorSetupOp(const std::string& name,
            const std::shared_ptr<const ClientCredentials>& cred, op_t op,
diff --git a/src/pvxs/srvcommon.h b/src/pvxs/srvcommon.h
index 4f11e38..0458828 100644
--- a/src/pvxs/srvcommon.h
+++ b/src/pvxs/srvcommon.h
@@ -93,8 +93,9 @@ struct PVXS_API ExecOp : public OpBase {
     virtual void reply() =0;
     //! Issue a reply with data.  For a GET or RPC  (or PUT/Get)
     virtual void reply(const Value& val) =0;
     //! Indicate the request has resulted in an error.
+    //! @since UNRELEASED Does not block
     virtual void error(const std::string& msg) =0;
 
     //! Callback invoked if the peer cancels the operation before reply() or error() is called.
     virtual void onCancel(std::function<void()>&&) =0;
diff --git a/src/serverget.cpp b/src/serverget.cpp
index 4f5053b..2441b6b 100644
--- a/src/serverget.cpp
+++ b/src/serverget.cpp
@@ -202,9 +202,10 @@ struct ServerGPRConnect : public server::ConnectOp
             throw std::invalid_argument("Must provide error message");
         auto serv = server.lock();
         if(!serv)
             return;
-        serv->acceptor_loop.call([this, &msg](){
+        auto op(this->op);
+        serv->acceptor_loop.dispatch([op, msg](){
             if(auto oper = op.lock()) {
                 if(oper->state==ServerOp::Creating)
                     oper->doReply(Value(), msg);
             }
@@ -272,9 +273,10 @@ struct ServerGPRExec : public server::ExecOp
     {
         auto serv = server.lock();
         if(!serv)
             return;
-        serv->acceptor_loop.call([this, &val](){
+        auto op(this->op);
+        serv->acceptor_loop.dispatch([op, val](){
             if(auto oper = op.lock()) {
                 oper->doReply(val, std::string());
             }
         });
@@ -286,9 +288,10 @@ struct ServerGPRExec : public server::ExecOp
             throw std::invalid_argument("Must provide error message");
         auto serv = server.lock();
         if(!serv)
             return;
-        serv->acceptor_loop.call([this, &msg](){
+        auto op(this->op);
+        serv->acceptor_loop.dispatch([op, msg](){
             if(auto oper = op.lock()) {
                 oper->doReply(Value(), msg);
             }
         });
diff --git a/src/servermon.cpp b/src/servermon.cpp
index 463506d..d8c6f6b 100644
--- a/src/servermon.cpp
+++ b/src/servermon.cpp
@@ -398,12 +398,13 @@ struct ServerMonitorSetup : public server::MonitorSetupOp
             throw std::invalid_argument("Must provide error message");
         auto serv = server.lock();
         if(!serv)
             return;
-        serv->acceptor_loop.call([this, &msg](){
+        auto op(this->op);
+        serv->acceptor_loop.dispatch([op, msg]() mutable {
             if(auto oper = op.lock()) {
                 if(oper->state==ServerOp::Creating) {
-                    oper->msg = msg;
+                    oper->msg = std::move(msg);
                     oper->doReply();
                 }
             }
         });

```
