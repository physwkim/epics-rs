...
use epics_base_rs::server::iocsh::{self, registry::CommandDef};
use epics_base_rs::server::{autosave, access_security, DeviceSupportFactory};
    ...
    for name in names {
        if let Some(rec_arc) = db.get_record(&name).await {
            ...
            if instance.record.record_type() == "sub" {
                if let Some(epics_base_rs::types::EpicsValue::String(snam)) =
