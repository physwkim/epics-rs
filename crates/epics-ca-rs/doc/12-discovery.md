...
- RFC 2136 dynamic DNS UPDATE (server self-registration to a
  unicast DNS): 🚧 follow-up — currently the operator must add the
  zone records manually (or use the snippet generator + script)
- Custom backends (Consul/etcd/CMDB plug-ins): trait surface ready,
  no bundled implementations
