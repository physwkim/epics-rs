...
fn test_get_put_scalar_fields() {
    ...
    rec.put_field("EGU", EpicsValue::String("counts".into())).unwrap();
    assert_eq!(rec.get_field("EGU"), Some(EpicsValue::String("counts".into())));
}
...
fn test_get_put_indexed_nm() {
    let mut rec = ScalerRecord::default();
    rec.put_field("NM1", EpicsValue::String("clock".into())).unwrap();
    assert_eq!(rec.get_field("NM1"), Some(EpicsValue::String("clock".into())));

    rec.put_field("NM10", EpicsValue::String("det1".into())).unwrap();
    assert_eq!(rec.get_field("NM10"), Some(EpicsValue::String("det1".into())));
}
...
fn test_type_mismatch() {
    ...
    assert!(rec.put_field("PR1", EpicsValue::String("bad".into())).is_err());
...
fn test_val_set_on_completion() {
    ...
    assert_eq!(rec.ss, 0); // IDLE
    assert!((rec.val - 1.0).abs() < 1e-6, "VAL should be ~1.0, got {}", rec.val);
}
