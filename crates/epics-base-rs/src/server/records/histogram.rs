...
static HISTOGRAM_FIELDS: &[FieldDesc] = &[
    ...
    FieldDesc { name: "NELM", dbf_type: DbFieldType::Long, read_only: true },
    FieldDesc { name: "ULIM", dbf_type: DbFieldType::Double, read_only: false },
    FieldDesc { name: "LLIM", dbf_type: DbFieldType::Double, read_only: false },
    FieldDesc { name: "SGNL", dbf_type: DbFieldType::String, read_only: false },
    FieldDesc { name: "CMD", dbf_type: DbFieldType::Short, read_only: false },
    FieldDesc { name: "SDEL", dbf_type: DbFieldType::Double, read_only: false },
];
...
impl Record for HistogramRecord {
    ...
    fn put_field(&mut self, name: &str, value: EpicsValue) -> CaResult<()> {
        match name {
            "VAL" => match value {
                EpicsValue::LongArray(arr) => { self.val = arr; Ok(()) }
                _ => Err(CaError::TypeMismatch("VAL".into())),
            },
            "ULIM" => match value { EpicsValue::Double(v) => { self.ulim = v; Ok(()) } _ => Err(CaError::TypeMismatch("ULIM".into())) },
            "LLIM" => match value { EpicsValue::Double(v) => { self.llim = v; Ok(()) } _ => Err(CaError::TypeMismatch("LLIM".into())) },
            "SGNL" => match value { EpicsValue::String(s) => { self.sgnl = s; Ok(()) } _ => Err(CaError::TypeMismatch("SGNL".into())) },
            "CMD" => match value { EpicsValue::Short(v) => { self.cmd = v; Ok(()) } _ => Err(CaError::TypeMismatch("CMD".into())) },
            "SDEL" => match value { EpicsValue::Double(v) => { self.sdel = v; Ok(()) } _ => Err(CaError::TypeMismatch("SDEL".into())) },
            "NELM" => Err(CaError::ReadOnlyField(name.to_string())),
            _ => Err(CaError::FieldNotFound(name.to_string())),
        }
    }

    fn field_list(&self) -> &'static [FieldDesc] { HISTOGRAM_FIELDS }
