...
# 0.5 um/pixel, +/-0.256 mm screen
epicsEnvSet("XRT_SCREEN_NX",  "1024")
epicsEnvSet("XRT_SCREEN_NZ",  "1024")
epicsEnvSet("XRT_SCREEN_DX",  "0.256")
epicsEnvSet("XRT_SCREEN_DZ",  "0.256")
epicsEnvSet("XRT_SIZE_X",     "1024")
epicsEnvSet("XRT_SIZE_Y",     "1024")
...
# DVAL=6.1 → 8 keV at period=10mm
dbLoadRecords("$(MOTOR)/motor.template", "P=$(PREFIX),M=und:gap,PORT=und_gap,VELO=5,ACCL=0.5,HLM=200,LLM=5")
dbLoadRecords("$(MOTOR)/motor.template", "P=$(PREFIX),M=und:x,PORT=und_x,VELO=1,ACCL=0.2,HLM=5,LLM=-5")
dbLoadRecords("$(MOTOR)/motor.template", "P=$(PREFIX),M=und:z,PORT=und_z,VELO=1,ACCL=0.2,HLM=5,LLM=-5")

# ===== Load DCM motor records =====
# DVAL=14.31 → Si(111) Bragg angle for 8 keV
dbLoadRecords("$(MOTOR)/motor.template", "P=$(PREFIX),M=dcm:theta,PORT=dcm_theta,VELO=1,ACCL=0.5,HLM=80,LLM=4")
dbLoadRecords("$(MOTOR)/motor.template", "P=$(PREFIX),M=dcm:theta2,PORT=dcm_theta2,VELO=10,ACCL=0.2,HLM=100,LLM=-100")
dbLoadRecords("$(MOTOR)/motor.template", "P=$(PREFIX),M=dcm:y,PORT=dcm_y,VELO=2,ACCL=0.3,HLM=50,LLM=0")
dbLoadRecords("$(MOTOR)/motor.template", "P=$(PREFIX),M=dcm:chi1,PORT=dcm_chi1,VELO=0.5,ACCL=0.2,HLM=5,LLM=-5")
dbLoadRecords("$(MOTOR)/motor.template", "P=$(PREFIX),M=dcm:chi2,PORT=dcm_chi2,VELO=0.5,ACCL=0.2,HLM=5,LLM=-5")
dbLoadRecords("$(MOTOR)/motor.template", "P=$(PREFIX),M=dcm:z,PORT=dcm_z,VELO=5,ACCL=0.3,HLM=200,LLM=-200")

# ===== Load HFM motor records =====
dbLoadRecords("$(MOTOR)/motor.template", "P=$(PREFIX),M=hfm:pitch,PORT=hfm_pitch,VELO=0.5,ACCL=0.5,HLM=10,LLM=1")
dbLoadRecords("$(MOTOR)/motor.template", "P=$(PREFIX),M=hfm:roll,PORT=hfm_roll,VELO=0.5,ACCL=0.2,HLM=5,LLM=-5")
dbLoadRecords("$(MOTOR)/motor.template", "P=$(PREFIX),M=hfm:yaw,PORT=hfm_yaw,VELO=0.5,ACCL=0.2,HLM=5,LLM=-5")
dbLoadRecords("$(MOTOR)/motor.template", "P=$(PREFIX),M=hfm:x,PORT=hfm_x,VELO=1,ACCL=0.2,HLM=10,LLM=-10")
...
dbLoadRecords("$(MOTOR)/motor.template", "P=$(PREFIX),M=vfm:roll,PORT=vfm_roll,VELO=0.5,ACCL=0.2,HLM=5,LLM=-5")
dbLoadRecords("$(MOTOR)/motor.template", "P=$(PREFIX),M=vfm:yaw,PORT=vfm_yaw,VELO=0.5,ACCL=0.2,HLM=5,LLM=-5")
dbLoadRecords("$(MOTOR)/motor.template", "P=$(PREFIX),M=vfm:x,PORT=vfm_x,VELO=1,ACCL=0.2,HLM=10,LLM=-10")
