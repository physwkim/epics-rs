...
impl XrtDetectorParams {
    ...
    pub fn motor_params(&self) -> Vec<usize> {
        vec![
            self.und_gap, self.und_x, self.und_z,
            self.dcm_theta, self.dcm_theta2, self.dcm_y,
            self.dcm_chi1, self.dcm_chi2, self.dcm_z,
            self.hfm_pitch, self.hfm_roll, self.hfm_yaw,
            self.hfm_x, self.hfm_y, self.hfm_z,
            self.hfm_r_major, self.hfm_r_minor,
            self.vfm_pitch, self.vfm_roll, self.vfm_yaw,
            self.vfm_x, self.vfm_y, self.vfm_z,
            self.vfm_r_major, self.vfm_r_minor,
        ]
    }
}
